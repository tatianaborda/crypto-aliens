...
// File contracts/CryptoAliensDNA.sol


pragma solidity ^0.8.0;

contract CryptoAliensDNA {
    ...
    function _deterministicPseudoRandomDNA(uint256 _tokenID, address _minter)
    public
    pure
    returns (uint256)
    {
...
// File contracts/CryptoAliens.sol


pragma solidity ^0.8.0;
contract CryptoAliens is ERC721, ERC721Enumerable, CryptoAliensDNA{
    ...
    constructor(uint256 _maxSupply) ERC721("CryptoAliens", "CRYAL"){
        maxSupply = _maxSupply;
    }

    function mint() public {
        uint256 current = _idCounter.current();
        require(current < maxSupply, "There is no crypto aliens left :( " );
        tokenDNA[current] = _deterministicPseudoRandomDNA(current, msg.sender);
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "CryptoAliens #',
                tokenId.toString(),
                '", "description": "Crypto Aliens are randomized Avataaars stored on chain to teach DApp development", "image": "',
                image,
                '"}'
            )
        );
