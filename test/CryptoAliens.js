...
describe("Crypto Aliens contract",()=>{
    const setup=async({maxSupply=10000})=>{
        const[owner]=await ethers.getSigners();
        const CryptoAliens=await ethers.getContractFactory("CryptoAliens");
        const deployed=await CryptoAliens.deploy(maxSupply);
...
describe("Minting",()=>{
    ...
    it("Has a minting limit",async()=>{
        ...
        await expect(deployed.mint()).to.be.revertedWith("There is no crypto aliens left :(");
    });
});
